...
1.	**Hierarchical Text Extraction**: Produces Text Info components (light orange).
2.	**Image Caption & Metadata Extraction**: Generates Image Info components (light blue).
3.	**LaTeX Equation Recognition**: Creates Equation Info components (light purple).
4.	**Table Structure & Content Parsing**: Produces Table Info components (light green).

#### Stage 2: Dual-Graph Construction for Multimodal Knowledge
This stage represents the core innovation of the framework, creating two complementary knowledge representations.

**Knowledge Graph Construction (Upper Dashed Box)**
•	**Text-Based Knowledge Graph**: Employs Entity & Relation Extraction processes (OpenAI, Ollama, etc.) to extract semantic entities (e.g., “Bee”, “Beekeeper”) and relationships (e.g., “Observe”).
•	**Cross-Modal Knowledge Graph**: Captures relationships between Parent Nodes (multimodal instances) and Child Nodes (component elements).
•	**Graph Merging**: Both graphs undergo a Merge operation to create a unified KG over All Documents.

**Vector Database Construction (Lower Dashed Box)**
•	**Encoding Process**: Text and multimodal information are processed through a Text Encoder to create separate Text and Multi-modal VDBs.
•	**Database Merging**: Merged into a comprehensive VDB over All Documents enabling semantic similarity searches across all content types.

#### Stage 3: Query Processing and Retrieval
The system accepts user queries and performs a dual retrieval strategy:
•	**Structural Knowledge Navigation**: Utilizes the constructed Knowledge Graph for graph-based reasoning and structural traversal.
•	**Semantic Similarity Matching**: Employs the Text Encoder to perform similarity searches against the Vector Database.

#### Stage 4: Response Generation
•	**Information Fusion**: The retrieval pathways converge to create **Hybrid Retrieved Info**, maintaining the original content type color coding.
•	**Synthesis**: A Large Language Model (LLM) synthesizes the retrieved multimodal information into a coherent, high-fidelity response.
